...
store_room(LServer, Host, Name, Opts) ->
    ...
    _ ->
            ...
            [<<"update muc_room set opts = '">>,SOpts,<<"' where name = '">>,Name,<<"' and host = '">>,Host,<<"';">>])
    end.
...
restore_room(LServer, Host, Name) ->
    case catch ejabberd_sql:sql_query(
     ...
     %            ?SQL("select @(opts)s from muc_room where name=%(Name)s"
      %                " and host=%(Host)s")) of
                [<<"select opts from muc_room where name= '">>,Name ,<<"' and host = '">>,Host,<<"';">>]) of
%	{selected, [{Opts}]} ->
	{selected,_ ,[[Opts]]} ->
...
%                      "where nick=%(Nick)s"
%                      " and host=%(Host)s")) of
         [<<"select jid from muc_registered where nick = '">>,Nick,<<"' and host = '">>,Host,<<"';">>]) of
	{selected,_, [[SJID1]]} -> SJID == SJID1;
	_ -> true
    end.
...
get_rooms(LServer, Host) ->
  ...
  %	{selected, RoomOpts} ->
                [<<"select name,opts from muc_room where host = '">>,Host,<<"';">>]) of
    {selected, _,RoomOpts} ->
	    lists:map(
	    %  fun({Room, Opts}) ->
            fun([Room, Opts]) ->
